...
public class MongoDBStorageProvider extends AbstractStorageProvider implements NoSqlStorageProvider {
    ...
    public static final String DEFAULT_DB_NAME = "schedules";
    ...
    public MongoDBStorageProvider(MongoClient mongoClient, String dbName, String collectionPrefix, DatabaseOptions databaseOptions, RateLimiter changeListenerNotificationRateLimit) {
        ...
        // Assuming no migration is required
        ...
        // Initializing the job mapper, if it not null. This is to handle pre-defined recurring job cases.
        JobMapper jobMapper = new JobMapper(determineJsonMapper());
        setJobMapper(jobMapper);

        // Setting up the mongo client reference for closing on destroy
        client = mongoClient;
    }
    ...
    public void setJobMapper(JobMapper jobMapper) {
        // Add check for null so that dupe heap objects don't get created.
        if (this.jobDocumentMapper == null) {
    ...
    public boolean recurringJobExists(String recurringJobId, StateName... states) {
        RecurringJob recurringJob = getRecurringJob(recurringJobId);
        ScheduleMetaData metaData = (ScheduleMetaData) recurringJob.getJobDetails().getJobParameters().get(0).getObject();

        if (metaData.isExecuteWithAlreadyRunning()) {
            return false;
        }

        return jobCollection.countDocuments(and(in(Jobs.FIELD_STATE, stream(states).map(Enum::name).collect(toSet())),
                eq(Jobs.FIELD_RECURRING_JOB_ID, recurringJobId))) > 0;

    }
    @Override
    public RecurringJob getRecurringJob(String recurringJobId) {
        ArrayList<RecurringJob> recurringJobs = recurringJobCollection.find(eq(toMongoId(Jobs.FIELD_ID), recurringJobId))
                .map(jobDocumentMapper::toRecurringJob)
                .into(new ArrayList<>());

        if (recurringJobs.size() > 1) {
            throw new JobRunrException(String.format("Duplicate Schedule ID's registered for id: %s", recurringJobId));
        }

        if (recurringJobs.size() == 0) {
            throw new JobRunrException(String.format("Asking to execute recurring job with unregistered id: %s", recurringJobId));
        }

        return recurringJobs.get(0);
    }
